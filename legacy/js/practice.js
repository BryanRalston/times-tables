(function (root, factory) {
  ...
  const SKILL_COPY = {
    ...
    mix: { title: "Mix", blurb: "+ ΓêÆ ├ù ├╖ together", list: "Mix (+ ΓêÆ ├ù ├╖)" },
  ...
  const TRAIL_CHAPTERS = [
    ...
    },
    {
      ...
      nodes: [
        ...
        { id: "multi-digit", title: "Multi-digit + ΓêÆ ├ù ├╖", playable: false },
      ],
    },
    {
      id: "grade-5",
  ...
  function makeSubtractQuestion(left, right) {
    ...
    return {
      ...
      operation: "subtract",
      symbol: "ΓêÆ",
      prompt: `${minuend} ΓêÆ ${subtrahend}`,
      fact: `${minuend} ΓêÆ ${subtrahend}`,
    };
  }
  ...
  function formatMissingSubtrahendQuestion(spec, first) {
    const token = "n";
    const prompt = `${spec.left} ΓêÆ ${token} = ${spec.result}`;
    return {
      topic: "subtrahend",
      operation: "subtract",
      symbol: "ΓêÆ",
      ...
      answerKind: "number",
      prompt,
      promptHtml: `${spec.left} ΓêÆ <span class="blank">${token}</span> = ${spec.result}`,
      fullPrompt: true,
      hint: first ? "n is hiding" : "What number is n?",
      fact: `${spec.left} ΓêÆ ${spec.answer} = ${spec.result}`,
      review: `${prompt}  so  n is ${spec.answer}`,
    };
  }
  ...
  function formatFactorQuestion(spec, first) {
    const token = "n";
    const prompt = `${spec.groups} ├ù ${token} = ${spec.total}`;
    return {
      topic: "factor",
      operation: "multiply",
      symbol: "├ù",
      ...
      answerKind: "number",
      prompt,
      promptHtml: `${spec.groups} ├ù <span class="blank">${token}</span> = ${spec.total}`,
      fullPrompt: true,
      hint: first ? "n is hiding" : "What number is n?",
      fact: `${spec.groups} ├ù ${spec.hidden} = ${spec.total}`,
      review: `${prompt}  so  n is ${spec.hidden}`,
    };
  }
  ...
  function whyModel(question) {
    ...
    if (question.topic === "subtrahend" && question.operation === "subtract") {
      return {
        ...
        slot: "right",
        token: question.token || "n",
        fact: `${question.left} ΓêÆ ${question.answer} = ${question.result}`,
      };
    }
    if (question.topic === "factor" && question.operation === "multiply") {
      return {
        ...
        slot: "right",
        token: question.token || "n",
        fact: `${question.groups} ├ù ${question.answer} = ${question.result}`,
      };
    }
    return null;
  }
  ...
  function formatOnestepQuestion(spec, first) {
    ...
    if (form === "n+a") {
      ...
      prompt = `n ΓêÆ ${a} = ${b}`;
      promptHtml = `${blank} ΓêÆ ${a} = ${b}`;
    } else if (form === "a-n") {
      prompt = `${a} ΓêÆ n = ${b}`;
      promptHtml = `${a} ΓêÆ ${blank} = ${b}`;
    } else if (form === "an") {
      prompt = `${a}n = ${b}`;
      promptHtml = `${a}${blank} = ${b}`;
    } else if (form === "n/a") {
    ...
    return {
      ...
      symbol: form === "an" ? "" : form === "n/a" || form === "a/n" ? "/" : form.includes("+") ? "+" : "ΓêÆ",
      kind: "onestep",
      form,
      left: a,
      right: b,
  ...
  function formatTwostepQuestion(spec, first) {
    ...
    if (form === "an+b") {
      ...
      prompt = `${a}n ΓêÆ ${b} = ${c}`;
      promptHtml = `${a}${blank} ΓêÆ ${b} = ${c}`;
    } else {
      prompt = `${a} ΓêÆ ${b}n = ${c}`;
      promptHtml = `${a} ΓêÆ ${b}${blank} = ${c}`;
    }
    return {
      topic: "twostep",
      operation: twostepOperation(form),
      symbol: form.includes("+") ? "+" : "ΓêÆ",
      kind: "twostep",
      form,
      a,
      b,
  ...
  function makeShapeQuestion(settings, rng) {
    ...
    if (kind === "area") {
      ...
      return {
        ...
        hint: "Count them, or multiply length ├ù width",
        answer,
        answerKind: "number",
        fact: `${length} ├ù ${width}`,
