...
| 2 | Leftover / what's hiding / find-n ten-frame let **Check** or **Skip** succeed with zero model taps. `needsInteract` must be true. Keypad, Check, and Skip stay hidden until the kid takes the known group. | `src/lib/answers-audit.test.ts` leftover ten-frames; `src/lib/year.test.ts` `welcomeFirst`; `src/lib/leftover.test.ts`. Playwright leftover Check disabled until a known-group tap, then Check works. |
