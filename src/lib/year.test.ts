...
describe("principal holes", () => {
  ...
  it("data cycle mutates a graph", () => {
    ...
    const other = d.rows.find((r) => r.label !== first.label)!.label;
    const miss = placeOnGraph(d.tray!, counts, first.id, other);
    expect(miss.ok).toBe(false);
    expect(miss.tray).toHaveLength(d.tray!.length);
    expect(miss.counts[other] ?? 0).toBe(0);
    expect(untieExtreme([3, 3, 2, 1], "max")).toEqual([4, 3, 2, 1]);
    expect(untieExtreme([3, 2, 1, 1], "min")).toEqual([3, 2, 0, 1]);
    expect(untieExtreme([3, 2, 2, 1], "max")).toEqual([3, 2, 2, 1]);
    expect(q.needsInteract).toBe(true);
...
describe("locale", () => {
  it("chrome strings cover English, Spanish, and Portuguese", () => {
    ...
    expect(UI.en.pickSquishee).toBe("Pick your squishee");
    expect(UI.es.wearIt).toBe("Poner");
    expect(UI["pt-BR"].dressUp).toBe("Chapéus e extras");
