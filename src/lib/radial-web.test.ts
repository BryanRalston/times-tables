...
describe("radial web", () => {
  ...
  it("keeps every hop off the #70 grass gaps and on a painted tile center", () => {
    ...
    for (const id of [15, 23, 38, 41, 59, 65, 85, 89, 105]) {
      expect(RADIAL_PADS.find((p) => p.id === id)?.portal).toBe(false);
    }
    const innerE = RADIAL_PADS.find((p) => p.portal && p.ring === 2 && p.map.x > 60)!;
    const innerW = RADIAL_PADS.find((p) => p.portal && p.ring === 2 && p.map.x < 40)!;
    expect(innerE?.id).toBe(96);
    ...
    expect(midSE.map.x).toBeGreaterThan(60);
    expect(midSE.map.x).toBeLessThan(63);
    expect(midSE.map.y).toBeGreaterThan(53);
    ...
    expect(midSW.portal).toBe(true);
    expect(midSW.map.x).toBeGreaterThan(37);
    expect(midSW.map.x).toBeLessThan(41);
    expect(midSW.map.y).toBeGreaterThan(54);
    ...
    const outerE = RADIAL_PADS.find((p) => p.portal && p.ring === 4 && p.map.x > 80)!;
    const outerW = RADIAL_PADS.find((p) => p.portal && p.ring === 4 && p.map.x < 18)!;
    const outerNW = RADIAL_PADS.find((p) => p.portal && p.ring === 4 && p.map.x < 30 && p.map.y < 18)!;
    expect(outerNE?.id).toBe(98);
    expect(outerE?.id).toBe(99);
    expect(outerW?.id).toBe(100);
    expect(outerNW?.id).toBe(101);
    expect(adjacentPadIds(98).length).toBeGreaterThan(0);
    expect(adjacentPadIds(99)).toEqual(expect.arrayContaining([65]));
    expect(adjacentPadIds(100)).toEqual(expect.arrayContaining([85]));
    expect(adjacentPadIds(101).length).toBeGreaterThan(0);
  });
  ...
  it("pairs portals as a hidden involution without self-warps", () => {
    ...
    for (const [a, b] of PORTAL_PAIRS) {
      ...
      expect(portalPartner(portalPartner(a)!)).toBe(a);
...
describe("radial hop hit testing", () => {
  ...
  it("a tap on a swirl lands on the vortex pad, not a cream neighbor", () => {
    const choices = adjacentPadIds(START_PAD);
    expect(choices).toContain(2);
    expect(choices).toContain(3);
    const swirl = padClientPos(2, board);
    expect(nearestHopTarget(swirl.x, swirl.y, board, choices, START_PAD)).toBe(2);
    const cream = padClientPos(3, board);
    expect(nearestHopTarget(cream.x, cream.y, board, choices, START_PAD)).toBe(3);
    const from = adjacentPadIds(2).find((id) => id !== START_PAD)!;
    const fromChoices = adjacentPadIds(from);
    expect(fromChoices).toContain(2);
    const p = padClientPos(2, board);
    expect(nearestHopTarget(p.x, p.y, board, fromChoices, from)).toBe(2);
  });
